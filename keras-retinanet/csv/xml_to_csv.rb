...
opt = {
  path: {
    annotation: nil,
    src_images: Pathname.new(File.absolute_path('./images')),
...
opt_parse.on('--val-ratio float', 'sample ratio for validation (0.0 - 1.0). default=0.2') {|v| opt[:val_ratio] = v.to_f}
opt_parse.on('--license_plate', 'create csv for license plates') {|v| opt[:path][:csv] = Pathname.new(File.absolute_path('./csv/license_plate')) }
opt_parse.on('--face', 'create csv for faces') {|v| opt[:path][:csv] = Pathname.new(File.absolute_path('./csv/face')) }
...
images_path = Dir.glob(opt[:path][:src_images] + '*.png')
...
# 
used_images = images_path.map{|p| [File.basename(p, '.png'), false] }.to_h
classes = []

objects = Dir.glob(opt[:path][:annotation] + '*.xml').map {|path_xml|
  basename = File.basename(path_xml, '.xml')
  path_png = opt[:path][:dest_images].relative_path_from(opt[:path][:csv]) + "#{basename}.png"

  objects = open_and_parse_xml(path_xml).map {|x1, y1, x2, y2, class_name|
    classes << class_name
    [path_png, x1, y1, x2, y2, class_name]
  }
...
objects = objects.flatten(1).sort_by{|path_png, x1, y1, x2, y2, class_name| [path_png, x1] }
objects_val = objects_val.flatten(1).sort_by{|path_png, x1, y1, x2, y2, class_name| [path_png, x1] }

un_used_images = used_images.select{|basename, flag|
                              flag == false
                            }
                            .map{|basename, flag|
                              relative_path = opt[:path][:dest_images].relative_path_from(opt[:path][:csv]) + "#{basename}.png"
                              [relative_path, "", "", "", "", ""]
                            }
